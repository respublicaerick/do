...
function obtenerInsumosR() {
    $bd = conectarBaseDatos();
    $sentencia = $bd->prepare("SELECT * FROM insumos");
    $sentencia->execute();
    return $sentencia->fetchAll(PDO::FETCH_OBJ);
}

function obtenerCategoriaR() {
    $bd = conectarBaseDatos();
    $sentencia = $bd->prepare("SELECT * FROM categorias");
    $sentencia->execute();
    return $sentencia->fetchAll(PDO::FETCH_OBJ);
}

function obtenerUsuariosR() {
    $bd = conectarBaseDatos();
...
function obtenerVentasR() {
    $bd = conectarBaseDatos();
    $sentencia = $bd->prepare("SELECT * FROM ventas");
    $sentencia->execute();
    return $sentencia->fetchAll(PDO::FETCH_OBJ);
}

function obtenerVentasInsumosR() {
	$bd = conectarBaseDatos();
	$sentencia = $bs->prepare("SELECT * FROM insumos_ventas");
	$sentencia->execute();
	return $sentencia->fetchAll(PDO::FETCH_OBJ);
}
...
function backupbase() {
    try {
        ...
        $db_user = "root";
        $db_password = "";
        $db_name = "botanero_ventas";
        $db_fecha = date("Ymd-His");
    
        $salida_sql = $db_name . '_' . $db_fecha . '.sql';
    
        $command = "mysqldump -h$db_host -u$db_user -p$db_password --opt $db_name > $salida_sql";
        ...
        if ($return_var === 0) {
            ...
            echo "Backup de la base de datos creado exitosamente";
        } else {
...
function recibe_excel_validando(){
	require('config.php');
